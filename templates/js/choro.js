...
choro = d3.json("https://raw.githubusercontent.com/raywhelan01/UCSD-Capstone/master/Maps/zillow_zip_data.json")
...
function onEachFeature(feature, layer) {
    ...
    if (feature.properties.MeanLRPrediction !== undefined) {
        
        // value of percent no internet for this county
        // use the d3-format library: https://github.com/d3/d3-format
        
        // define the popup content
        let content = `<div>ZIP Code: ${feature.properties.ZIP}</div>
                       <div>Mean Model Predictions</div>
                       <div>Linear Regression: ${feature.properties.MeanLRPrediction}</div>
                       <div>LR RMSE: ${feature.properties.MeanLRRmsePrediction}</div>
                       <div>Gradient Boosted: ${feature.properties.MeanGBPrediction}</div>
                       <div>GB RMSE: ${feature.properties.MeanGBRmsePrediction}</div>
                       <div>Random Forest: ${feature.properties.MeanRFPrediction}</div>
                       <div>RF RMSE: ${feature.properties.MeanRFRmsePrediction}</div>`
